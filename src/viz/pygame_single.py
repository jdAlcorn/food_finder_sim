...
def draw_vision(screen, sim, show_vision):
    ...
    for i, (distance, hit_type, wall_id, angle) in enumerate(zip(distances, hit_types, hit_wall_ids, angles)):
        ...
        if distance < sim.config.max_range:
            ...
            outside_bounds = (hit_x < -5 or hit_x > sim.config.world_width + 5 or 
                            hit_y < -5 or hit_y > sim.config.world_height + 5)
            
            if outside_bounds:
                # Draw red circle for hits outside bounds
                pygame.draw.circle(screen, (255, 0, 0), 
                                 (int(max(0, min(sim.config.world_width, hit_x))), 
                                  int(max(0, min(sim.config.world_height, hit_y)))), 4)
